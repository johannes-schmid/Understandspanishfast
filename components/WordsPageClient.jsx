    ...
    '@graph': [
      {
        ...
        publisher: { '@type': 'Organization', name: 'Most Common Spanish', logo: { '@type': 'ImageObject', url: 'https://mostcommonspanish.com/icon.svg' } },
      ...
      <main className="max-w-7xl mx-auto px-6 md:px-8 pt-32 pb-16">
        <header className="mb-10">
          <p className="text-sm text-on-surface-variant mb-2">
            <Link href="/" className="hover:text-[#FF8C00]">Home</Link> <span className="mx-1">/</span> Words
          </p>
          ...
          <div className="flex flex-col md:flex-row gap-3 md:items-center">
            <div className="relative flex-1">
              ...
              <Input
                ...
                className="pl-12 h-auto py-3 rounded-full border-2 border-surface-variant focus:border-[#FF8C00] focus-visible:ring-0 focus-visible:border-[#FF8C00] outline-none"
              />
            </div>
            <Button
              onClick={() => setHideTrans((v) => !v)}
              className={`h-auto px-5 py-3 rounded-full font-semibold transition-colors ${hideTrans ? 'bg-primary-container text-on-primary-container hover:bg-primary-container' : 'bg-surface-container-highest text-on-surface-variant hover:bg-surface-container-high'}`}
            >
          ...
          <div className="flex flex-wrap gap-2">
            {FILTERS.map((f) => (
              <Button
                ...
                className={`filter-chip h-auto px-4 py-2 rounded-full text-sm font-semibold transition-colors ${filter === f.key ? 'active text-white' : 'bg-surface-container-highest text-on-surface-variant hover:bg-surface-container-high'}`}
              >
                {f.label}
              </Button>
            ))}
                    ...
                    <FragmentRow key={w.rank}>
                      ...
                      <tr className={`word-row border-t border-surface-variant ${isLearned ? 'row-learned' : ''}`}>
                        ...
                        <td className="px-4 py-3">
                          <Badge className={`px-3 py-1 h-auto rounded-full text-xs font-semibold badge-${w.type}`}>
                        ...
                        <td className="px-4 py-3 text-right">
                          <Button
                            ...
                            className={`material-symbols-rounded bg-transparent border-none shadow-none h-auto p-0 ${isLearned ? 'text-[#FF8C00]' : 'text-surface-variant hover:text-[#FF8C00]'} transition-colors hover:bg-transparent`}
        ...
        <section className="mt-12 bg-secondary-container rounded-[2rem] p-10 text-center">
          ...
          <Link href="/level-test" className="inline-block bg-primary-container text-on-primary-container px-10 py-4 rounded-full font-semibold shadow-lg hover:scale-[1.02] transition-all no-underline">
            Test Your Level
          </Link>
        </section>
