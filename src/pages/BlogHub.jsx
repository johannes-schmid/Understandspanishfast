...
export const POSTS = [
  ...
  },
  {
    ...
    image: '/blog/cracking-1500-hero.svg',
  },
  {
    ...
    badge: 'Popular',
    readTime: '7 min read',
    image: '/blog/how-many-words-hero.svg',
  },
]
    ...
    <div className="bg-background text-on-background min-h-screen">
      ...
      <main className="max-w-7xl mx-auto px-6 md:px-8 pt-32 pb-16">
        ...
        <section className="grid grid-cols-1 lg:grid-cols-12 gap-6 mb-16">
          <Link to={`/blog/${featured.slug}`} className="lg:col-span-8 group cursor-pointer bg-white rounded-[2rem] overflow-hidden shadow-sm bento-card no-underline text-on-surface">
          ...
          <div className="lg:col-span-4 bg-primary-container text-on-primary-container p-8 rounded-[2rem] flex flex-col justify-between">
            ...
            <Link to="/level-test" className="bg-on-primary-fixed text-primary-fixed py-3 px-6 rounded-full font-semibold self-start hover:scale-95 transition-transform no-underline">
              Test your level
            </Link>
          </div>
        </section>
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-16">
          {rest.map((p) => (
            <Link key={p.slug} to={`/blog/${p.slug}`} className="bg-white rounded-[2rem] overflow-hidden bento-card group cursor-pointer no-underline text-on-surface">
        ...
        <section className="bg-secondary-container rounded-[2rem] p-8 md:p-12 flex flex-col md:flex-row items-center gap-8">
          ...
          <Link to="/#waitlist" className="bg-primary text-on-primary py-4 px-8 rounded-full font-semibold hover:bg-primary-container hover:text-on-primary-container transition-colors no-underline whitespace-nowrap">
            Join the waitlist
          </Link>
        </section>
      </main>
