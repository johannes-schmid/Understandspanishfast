...
export default function BlogPost({ slug, title, description, category, readTime, datePublished = '2026-05-01', dateModified = '2026-05-07', heroCallout, sidebar, children }) {
  ...
  return (
    <div className="bg-background text-on-background">
      <SEO title={title} description={description} canonical={`/blog/${slug}`} type="article" jsonLd={jsonLd} />

      <main className="max-w-7xl mx-auto px-6 md:px-8 pt-32 pb-16">
        <p className="text-sm text-on-surface-variant mb-4">
          <Link to="/blog" className="hover:text-[#FF8C00]">← Back to blog</Link>
        </p>
          ...
          <article className="lg:col-span-8 order-1 lg:order-2">
            ...
            <section className="bg-secondary-container rounded-[2rem] p-10 text-center mt-12 relative overflow-hidden">
              ...
              <div className="flex flex-col sm:flex-row justify-center gap-3">
                <Link to="/level-test" className="bg-primary-container text-on-primary-container px-8 py-3 rounded-full font-semibold shadow-sm hover:brightness-110 active:scale-95 transition-all no-underline">
                  Start Level Test
                </Link>
                <Link to="/#waitlist" className="bg-white text-secondary px-8 py-3 rounded-full font-semibold shadow-sm hover:bg-surface transition-all no-underline">
                  Join Waitlist
                </Link>
              </div>
            </section>
